function [ Raw ] = weighted_demosaic( Raw1C, WhiteImage, Belonging, DecodeOptions )
%WEIGHTED_DEMOSAIC Perform a white lenslet image guided demosaicing on
%a raw lenslet image
%   Raw = weighted_demosaic(Raw1C, WhiteImage, Belonging, DecodeOptions)
%   Raw: demosaiced lenslet image
%   Raw1C: raw lenslet image
%   WhiteImage: white lenslet image serving as guide for the demosaicing
...
WhiteImage = padarray(WhiteImage, [padsize,padsize],0);
...
% Raw(id + M*N) = sum(Gvals.*Gweight,2) + a.*min(4/5*(Rsum + WhiteImage(id))./Gsum,1).^2.*gradR;
...
% Raw(id + M*N*2) = sum(Bvals.*Bweight,2) + c.*min(4/5*(Rsum+ WhiteImage(id))./Bsum,1).^2.*gradR;
...
Gweight = Gconf.*Gbelong./repmat(Gsum,1,4);
Gweight(~isfinite(Gweight)) = 0.25;
% Raw(id + M*N) = sum(Gvals.*Gweight,2) + a*min(4/5*(Bsum + WhiteImage(id))./Gsum,1).^2.*gradB;
...
Rvals = Raw1C(id_valR);
Rconf = WhiteImage(id_valR);
Rbelong = Belonging(id_valR) == repmat(Belonging(id),1,4);
Rsum = sum(Rconf.*Rbelong,2);
Rweight = Rconf.*Rbelong./repmat(Rsum,1,4);
Rweight(~isfinite(Rweight)) = 0.25;
% Raw(id) = sum(Rvals.*Rweight,2) + c*min(4/5*(Bsum + WhiteImage(id))./Rsum,1).^2.*gradB;
